ativaMenu('menu_mol_adm_controleacesso',true,[['menu_mol_adm_controleacesso_acessopaginas','flush-collapseControleAcesso']])
...
function listarGrupoUsuario(){
  ...
  fazPost(URL_API,obj,"consultaMenuDoGrupoUsuario",retornoConsultarMenuDoGrupoUsuario,false,)
}

function retornoConsultarMenuDoGrupoUsuario(grupoUsuarioRetornados,erro){
  //console.log(grupoUsuarioRetornados)
  preencheMenuGrupo(grupoUsuarioRetornados['MENU_ASSOCIADO'],'tabelaListaPaginasPermitidas')
  preencheMenuGrupo(grupoUsuarioRetornados['MENU_NAO_ASSOCIADO'],'tabelaListaPaginasNaoPermitidas')
  let area_paginas_grupo = document.getElementById('area_paginas_grupo')
  let area_lista_grupos = document.getElementById('area_lista_grupos')
  
  area_lista_grupos.classList.add('visually-hidden')
  area_paginas_grupo.classList.remove('visually-hidden')


}

function voltaListaBarraca(){
  let area_paginas_grupo = document.getElementById('area_paginas_grupo')
  let area_lista_grupos = document.getElementById('area_lista_grupos')

  area_lista_grupos.classList.remove('visually-hidden')
  area_paginas_grupo.classList.add('visually-hidden')
}

function preencheMenuGrupo(grupoUsuario,idTabela){
  let ehAssoc = false
  let nivelGrupo = document.querySelector('input[name=nivelGrupo]:checked').value
  if(idTabela == 'tabelaListaPaginasPermitidas')
    ehAssoc = true
...
function associaGrupoUsuarioAoMenu(idGrupo,nivelGrupo,classe){
  ...
  fazPost(URL_API,obj,"associaGrupoUsuarioAoMenu",retornoAssociaGrupoUsuarioAoMenu,false)
}

function retornoAssociaGrupoUsuarioAoMenu(objetosRetornados,erro){
  exibeMsgSucesso("Menu Foi Associado Com Sucesso!")
...
function excluirGrupoUsuario(idGrupo,nivelGrupo,classe){
  ...
  fazPost(URL_API,obj,"excluirGrupoUsuario",retornoExcluirGrupoUsuario,false)
}

function retornoExcluirGrupoUsuario(objetosRetornados,erro){
  exibeMsgSucesso("Menu Foi Desassociado Com Sucesso!")
